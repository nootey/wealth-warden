...
import {Chart as ChartJS, ArcElement, Tooltip, Legend, PieController, type ChartOptions} from "chart.js";
...
const chartData = ref<ChartData<"pie">>({
    labels: props.labels,
    datasets: [
        { data: props.values, backgroundColor: ["#36A2EB","#FF6384","#FFCE56","#4BC0C0","#9966FF","#FF9F40"] }
    ]
});

const generateColors = (count: number) => {
    const colors = ["#36A2EB","#FF6384","#FFCE56","#4BC0C0","#9966FF","#FF9F40"];
    while (colors.length < count) colors.push(`#${Math.floor(Math.random()*16777215).toString(16)}`);
    return colors.slice(0, count);
};

watch([() => props.values, () => props.labels], ([vals, labs]) => {
    chartData.value = {
        labels: labs,
...
function merge<T>(base: any, extra: any): T {
    ...
    for (const k in extra) {
        const bv = (base ?? {})[k];
        const ev = extra[k];
        out[k] = (bv && typeof bv === "object" && !Array.isArray(bv) && typeof ev === "object" && !Array.isArray(ev))
            ? merge(bv, ev)
            : ev;
    }
    return out as T;
...
const baseOptions = computed<ChartOptions<"pie">>(() => ({
    ...
    animation: { animateScale: true, animateRotate: true, duration: 1500, easing: "easeOutCubic" },
    plugins: {
