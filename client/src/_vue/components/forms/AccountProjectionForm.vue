...
onMounted(async () => {
  ...
  account.value = await sharedStore.getRecordByID(
    "accounts",
...
const expectedBalanceNumber = currencyHelper.useMoneyField(
  expectedBalanceFieldRef,
  2,
).number;
...
const expectedBalance = computed(() => {
  ...
  if (record.value.balance_projection === "multiplier") {
    ...
    if (
      record.value.expected_balance &&
      record.value.expected_balance !== "0"
    ) {
      try {
...
watch(effectiveAccountID, async (newID, oldID) => {
  ...
  if (accountIDToUse && accountIDToUse !== oldID) {
    const newAccount = await sharedStore.getRecordByID(
      "accounts",
      accountIDToUse,
    );
    if (newAccount) {
...
watch(
  () => record.value.multiplier_category_id,
  async (newCategoryId) => {
      ...
      <div class="flex flex-column gap-2 p-3 w-full">
        ...
        >
          ...
          <span class="text-sm" style="color: var(--text-secondary)">
            Input a fixed balance. This value will be used as the expected
        ...
        >
          ...
          <span class="text-sm" style="color: var(--text-secondary)">
            Input a percentage rate. This value will be used as the expected
            growth for this account. The growth period is currently unlimited.
          </span>
        </div>
        ...
        >
          ...
          <span class="text-sm" style="color: var(--text-secondary)">
            Select a category or group and input a multiplier. The expected
            balance will be the monthly average of transactions, linked to this
            category, constrained to the current year - with the provided
            multiplier.
          </span>

          <span class="text-sm" style="color: var(--text-secondary)">
            For example, you can define a 6 times multiplier and select your
            salary as the category, which would set the expected balance as 6
