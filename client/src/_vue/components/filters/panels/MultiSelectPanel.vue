...
const normalizedOptions = computed(() => {
  ...
  if (typeof opts[0] !== "object") {
    return (opts as (string | number)[]).map((v) => ({
      label: String(v),
      value: v,
      raw: v,
    }));
  ...
  const ov = props.optionValue ?? ("id" in (opts[0] as any) ? "id" : ol);
...
const labelMap = computed(
  () => new Map(normalizedOptions.value.map((o) => [o.value, o.label])),
);
const selectedChips = computed(() =>
  internal.value.map((v) => ({
    value: v,
    label: labelMap.value.get(v) ?? String(v),
  })),
);
