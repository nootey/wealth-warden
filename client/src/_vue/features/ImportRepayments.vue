...
async function fetchValidationResponse(importId: number) {
    ...
    try {
        validatedResponse.value = await dataStore.getCustomImportJSON(importId, "repayment");
...
async function transferRepayments() {
    ...
    if (!selectedImport.value?.id) {
        toastStore.errorResponseToast({"title": "Error", "message": "Missing import ID"});
        return;
    }

    if (Object.keys(repaymentMappings.value).length === 0) {
        toastStore.errorResponseToast({"title": "Error", "message": "Please set up your repayment mappings first"})
        return
    }
    ...
    if (!selectedCheckingAcc.value?.id) {
        toastStore.errorResponseToast({"title": "Error", "message": "No checking account"});
        return;
    }
...
<template>
  ...
  >
    ...
    <div
      ...
      class="flex flex-column w-full p-2"
    >
    ...
    >
      <span style="color: var(--text-secondary)">No repayments were found in the provided import!</span>
    </div>
