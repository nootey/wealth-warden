...
const rules = {
  form: {
    selectedYears: {
      required: helpers.withMessage("Select at least one year", required),
      $autoDirty: true,
    },
    inflowCategories: {
      atLeastOne: helpers.withMessage(
        "Select at least one inflow or outflow category",
        () =>
...
<template>
  <div class="w-full flex flex-column gap-3 p-2">
    <div
      id="report-filters"
      class="flex flex-row gap-3 align-items-end flex-wrap"
    >
      ...
      <div class="flex flex-column justify-content-end" style="flex: 0 0 auto">
        <Button
          class="main-button"
          label="Generate"
