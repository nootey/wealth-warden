...
const DatePanel = defineAsyncComponent(
  () => import("../_vue/components/filters/panels/DatePanel.vue"),
);
const MultiSelectPanel = defineAsyncComponent(
  () => import("../_vue/components/filters/panels/MultiSelectPanel.vue"),
);
const RangePanel = defineAsyncComponent(
  () => import("../_vue/components/filters/panels/RangePanel.vue"),
);
...
export const defs = {
  ...
  enumMulti(col?: Column): PanelDef<string[] | number[] | null> {
    ...
    const inferredValueKey = inferOptionValueKey(
      options,
      optionLabel,
      col?.optionValue,
    );
          ...
          : [
              {
                ...
                field,
                operator: "like",
                value: String(v).trim().replace(/\s+/g, " "),
              },
            ],
    };
  },
...
function inferOptionValueKey(
  ...
  if (options.every((o) => typeof o === "object" && o && "id" in o))
    return "id";

  if (
    optionLabel &&
    options.every((o) => typeof o === "object" && o && optionLabel in o)
  ) {
    return optionLabel;
  }
  ...
  for (const k of candidateKeys) {
    if (
      options.every(
        (o) =>
          o && k in o && ["string", "number", "boolean"].includes(typeof o[k]),
      )
    ) {
      return k;
    }
