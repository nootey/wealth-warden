...
type TransactionRepositoryInterface interface {
	...
	DeleteCategoryGroup(ctx context.Context, tx *gorm.DB, id, userID int64) (int64, error)
...
func (r *TransactionRepository) FindTransactionByExternalID(ctx context.Context, tx *gorm.DB, accountID int64, externalID string) (models.Transaction, error) {
	db := tx
	if db == nil {
		db = r.db
	}
...
func (r *TransactionRepository) FindAllCategoryGroups(ctx context.Context, tx *gorm.DB, userID int64) ([]models.CategoryGroup, error) {
	...
	q := db.Model(&models.CategoryGroup{}).
		Preload("Categories").
		Where("user_id = ?", userID).
		Order("classification, name").
		Find(&records)
...
func (r *TransactionRepository) UpdateCategoryGroup(ctx context.Context, tx *gorm.DB, record models.CategoryGroup) (int64, error) {
	...
	res := db.Model(models.CategoryGroup{}).
		Where("id = ?", record.ID).
		Updates(map[string]interface{}{
			"name":           record.Name,
			"classification": record.Classification,
...
func (r *TransactionRepository) DeleteCategoryGroup(ctx context.Context, tx *gorm.DB, id, userID int64) (int64, error) {
	db := tx
	if db == nil {
		db = r.db
	}
