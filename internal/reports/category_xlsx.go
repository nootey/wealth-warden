...
func BuildCategoryXLSX(rows []models.CategoryReportDataRow, params models.CategoryReportParams, scopeLabel string) (data []byte, err error) {
	...
	writeSummarySheet(f, "Summary", styles, rows, years, scopeLabel)

	if params.AllTime && len(years) > 1 {
		if _, err := f.NewSheet("All Time"); err != nil {
			return nil, err
		}
		writeAllTimeSheet(f, "All Time", styles, rows, years)
	}
	...
	for _, year := range years {
		...
		writeYearSheet(f, name, styles, year, byYear[year])
	}
...
func makeStyles(f *excelize.File) xlsxStyles {
...
func writeSummarySheet(f *excelize.File, sheet string, styles xlsxStyles, rows []models.CategoryReportDataRow, years []int, scopeLabel string) {
...
func writeYearSheet(f *excelize.File, sheet string, styles xlsxStyles, year int, rows []models.CategoryReportDataRow) {
	...
	addYearCharts(f, sheet, months, sumHeaderRow, primarySumRow, secondarySumRow, effectiveSumRow, statsStartRow, len(outflows) > 0, primaryLabel)
...
func writeAllTimeSheet(f *excelize.File, sheet string, styles xlsxStyles, rows []models.CategoryReportDataRow, years []int) {
	...
	addAllTimeChart(f, sheet, years, annualHeaderRow, effectiveYearRow, allTimeStatsRow, primaryLabel)
