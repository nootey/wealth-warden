...
func AccountRoutes(apiGroup *gin.RouterGroup, handler *handlers.AccountHandler) {
	...
	apiGroup.GET("/name/:name", authz.RequireAllMW("view_data"), handler.GetAccountByName)
	...
	apiGroup.PATCH("/defaults/set/:id", authz.RequireAllMW("manage_data"), handler.SetDefaultAccount)
	apiGroup.PATCH("/defaults/unset/:id", authz.RequireAllMW("manage_data"), handler.UnsetDefaultAccount)
}
