...
func (s *LoggingServiceTestSuite) TestFetchAuditTrail_ScopedToCauser() {
	...
	trail, err := svc.FetchAuditTrail(s.Ctx, "42", []string{"account"}, []string{"update"}, owner)
	s.Require().NoError(err)
...
func (s *LoggingServiceTestSuite) TestFetchAuditTrail_SpansCategoriesOnSubjectID() {
	...
	trail, err := svc.FetchAuditTrail(s.Ctx, "7", []string{"account", "balance"}, []string{"update"}, owner)
	s.Require().NoError(err)
...
func (s *LoggingServiceTestSuite) TestFetchAuditTrail_ForeignRecordReturnsEmpty() {
	...
	trail, err := svc.FetchAuditTrail(s.Ctx, "99", []string{"account"}, []string{"update"}, owner)
	s.Require().NoError(err)
