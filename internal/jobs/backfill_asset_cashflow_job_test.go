...
func runBackfill(w *jobs.BackfillAssetCashFlowsWorker, ctx context.Context) error {
	return w.Work(ctx, &river.Job[jobqueue.BackfillAssetCashFlowsArgs]{
...
func TestBackfillAssetCashFlowsJob_NoUsers(t *testing.T) {
	worker := jobs.NewBackfillAssetCashFlowsWorker(
		zaptest.NewLogger(t),
...
func TestRebuildJobsShareTheSingleWorkerQueue(t *testing.T) {
	...
	for _, a := range args {
		...
		if got := withOpts.InsertOpts().Queue; got != jobqueue.QueueRebuild {
			t.Errorf("%s runs on queue %q, want %q", a.Kind(), got, jobqueue.QueueRebuild)
		}
	}
}
