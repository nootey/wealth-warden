...
func NewAutomateTemplateJob(logger *zap.Logger, container *bootstrap.ServiceContainer, notifDispatcher jobqueue.NotificationDispatcher, concurrentWorkers int) *AutomateTemplateJob {
...
func (j *AutomateTemplateJob) Run(ctx context.Context) error {
	...
	runPhase := func(phase []*models.TransactionTemplate, out chan<- result) {
		runPool(ctx, phase, j.concurrentWorkers, "templates", func(ctx context.Context, tmpl *models.TransactionTemplate) error {
	...
	successCount := 0
	alreadyRan := 0
	failures := newPoolResult(len(templates), "templates")
	...
	failures.logFailures(j.logger, "Failed to process template")
