...
type Transaction struct {
	...
	Direction       TransactionDirection `gorm:"not null;enum(income,expense)" json:"direction"`
...
func NewOpeningTransaction(userID, accountID int64, categoryID *int64, currency string, openedAt time.Time, amount decimal.Decimal) Transaction {
	direction := TxnDirectionIncome
	if amount.IsNegative() {
		direction = TxnDirectionExpense
		amount = amount.Neg()
	}
...
type TransactionTemplate struct {
	...
	Direction    *TransactionDirection `gorm:"enum(income,expense)" json:"direction,omitempty"`
	Amount       decimal.Decimal       `gorm:"type:decimal(19,4);not null" json:"amount"`
...
type TransactionReq struct {
	...
	CategoryID     *int64               `json:"category_id,omitempty"`
	Direction      TransactionDirection `json:"direction" validate:"required"`
