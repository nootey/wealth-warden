...
func TestRuleMatches(t *testing.T) {
	...
	cases := []struct {
		...
		{"direction equals expense matches", all(cond("direction", "equals", "expense")), TxnDirectionExpense, true},
		{"direction equals expense fails on income", all(cond("direction", "equals", "expense")), TxnDirectionIncome, false},
		{"direction with gt operator is inert", all(cond("direction", "gt", "expense")), TxnDirectionExpense, false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := tc.rule.Matches("SPAR LJUBLJANA - nakup", amt, tc.dir); got != tc.want {
