...
func TestRecurringTransactions_FundsGoalsAfterTemplates(t *testing.T) {
	...
	worker := scheduler_jobs.NewRecurringTransactionsWorker(zap.NewNop(),
	...
	require.NoError(t, worker.Work(context.Background(), &river.Job[scheduler_jobs.RecurringTransactionsArgs]{}))
...
func TestRecurringTransactions_SkipsGoalsWhenTemplatesFail(t *testing.T) {
	...
	err := worker.Work(context.Background(), &river.Job[scheduler_jobs.RecurringTransactionsArgs]{})
...
func TestRecurringTransactions_SkipsGoalsWhenCancelled(t *testing.T) {
	...
	err := worker.Work(ctx, &river.Job[scheduler_jobs.RecurringTransactionsArgs]{})
	require.False(t, funded)
