...
func periodicSpecs(cfg config.SchedulerConfig) []periodicSpec {
	...
	return []periodicSpec{
		{dailyAt{0, 0}, scheduler_jobs.AssetPriceHistoryBackfillArgs{}, immediate["asset_history_backfill"]},
		{dailyAt{0, 6}, scheduler_jobs.BalanceBackfillArgs{}, immediate["balance_backfill"]},
		{dailyAt{0, 10}, scheduler_jobs.RecurringTransactionsArgs{}, immediate["recurring_transactions"]},
		{river.PeriodicInterval(8 * time.Hour), scheduler_jobs.AssetPriceSyncArgs{}, immediate["asset_price_sync"]},
	}
}
