...
-- Allow same (user_id, name) to be reused once a prior account is deleted
...
-- Cannot be both enabled and deleted
...
CREATE TRIGGER trg_accounts_state_guard
    BEFORE INSERT OR UPDATE OF is_active, deleted_at ON accounts
    FOR EACH ROW
